    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-dark-700 rounded-lg p-3">
                  <div className="text-dark-400 text-xs mb-1">Pan Angle</div>
                  <div className="text-2xl font-mono text-white">{servo.pan_angle.toFixed(1)}°</div>
                </div>
                <div className="bg-dark-700 rounded-lg p-3">
                  <div className="text-dark-400 text-xs mb-1">Tilt Angle</div>
                  <div className="text-2xl font-mono text-white">{servo.tilt_angle.toFixed(1)}°</div>
                </div>
              </div>
              <div className="flex items-center gap-2 text-sm">
                <div className={`w-2 h-2 rounded-full ${servo.connected ? 'bg-green-500' : 'bg-red-500'}`} />
        ...
        <div className={`rounded-xl p-6 border ${
          ...
          <div className="space-y-4">
            <div className="bg-dark-700 rounded-lg p-3">
              <div className="text-dark-400 text-xs mb-1">Host</div>
              <div className="text-white font-mono">{ptz.host}</div>
            </div>
            <div className="flex items-center gap-2 text-sm">
              <div className={`w-2 h-2 rounded-full ${ptz.connected ? 'bg-green-500' : 'bg-dark-500'}`} />
              <span className="text-dark-300">
      ...
      <div className="bg-dark-800 rounded-xl p-6 border border-dark-600">
        <h3 className="text-sm text-dark-400 uppercase tracking-wider mb-3">Current Output</h3>
        <div className="flex items-center gap-3">
          <div className={`w-3 h-3 rounded-full ${
            outputMode === 'virtual' ? 'bg-blue-500' :
            outputMode === 'servo' ? 'bg-green-500' :
            outputMode === 'hybrid' ? 'bg-yellow-500' :
            'bg-purple-500'
          }`} />
